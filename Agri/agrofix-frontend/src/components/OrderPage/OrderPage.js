  ...
  return (
    <div className="order-main">
      <div className="order-title">
        <AddShoppingCartIcon className="order-title-icon" /> {t('placeOrder') || 'Place Order'}
      </div>
      ...
      {cart.items.length === 0 ? (
        <div className="order-empty-message">
          ...
          <div style={{ fontSize: 16, color: '#388e3c', marginBottom: 18 }}>
            {t('addItemsToCart') || 'Browse our catalogue and add items to your basket to place an order.'}
          </div>
        ...
        <div className="order-cart-list order-cart-card">
          <div className="order-cart-header">{t('cart') || 'Cart'}</div>
          {cart.items.map(item => (
      ...
      {cart.items.length > 0 && (
        <form className="order-form" onSubmit={handleSubmit(onSubmit)} autoComplete="off" noValidate>
          <label htmlFor="buyer_name">{t('name') || 'Name'}</label>
          <input id="buyer_name" {...register('buyer_name')} />
          {errors.buyer_name && <div className="order-error">{errors.buyer_name.message}</div>}

          <label htmlFor="buyer_contact">{t('contact') || 'Contact'}</label>
          <input id="buyer_contact" {...register('buyer_contact')} />
          {errors.buyer_contact && <div className="order-error">{errors.buyer_contact.message}</div>}

          <label htmlFor="delivery_address">{t('deliveryAddress') || 'Delivery Address'}</label>
          ...
          <button type="submit" className="btn-primary order-submit-btn">{t('placeOrder') || 'Place Order'}</button>
