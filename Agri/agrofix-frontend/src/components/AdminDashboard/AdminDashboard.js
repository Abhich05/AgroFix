  ...
  return (
    <Box className="admin-main" sx={{ maxWidth: 1200, mx: 'auto', my: 4, background: '#fffbe7', borderRadius: 3, boxShadow: 3, p: { xs: 2, sm: 4 } }}>
      ...
      <Tabs value={tab} onChange={(_, v) => setTab(v)} centered sx={{ mb: 3 }}>
        <Tab icon={<ListAltIcon />} label={t('orders') || 'Orders'} />
        <Tab icon={<Inventory2Icon />} label={t('products') || 'Products'} />
      </Tabs>
      {tab === 0 && (
      ...
      {tab === 1 && (
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', mb: 2, justifyContent: 'space-between' }}>
          <Typography sx={{ fontWeight: 700, color: '#388e3c', fontSize: 22, letterSpacing: 1 }}>
            {t('products') || 'Products'}: {inventory.length}
          </Typography>
          <Button
            variant="contained"
            sx={{ background: '#388e3c', fontWeight: 700, borderRadius: 2, fontSize: 16, letterSpacing: 1 }}
            ...
            <TableBody>
              {inventory.map(product => (
                <TableRow key={product.id}>
                  ...
                  <TableCell>
                    <Button variant="contained" size="small" sx={{ mr: 1, mb: 0.5, background: '#0288d1', borderRadius: 2, fontWeight: 700 }} onClick={() => editInventory(product.id, product)}>{t('edit') || 'Edit'}</Button>
                    <Button variant="contained" size="small" sx={{ background: '#b71c1c', borderRadius: 2, fontWeight: 700 }} onClick={() => deleteInventory(product.id)}>{t('delete') || 'Delete'}</Button>
                  </TableCell>
                </TableRow>
              ))}
            </TableBody>
